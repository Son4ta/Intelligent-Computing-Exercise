...
# 贝叶斯二分类优化 重点
bayes_false = [np.ones(image_size) for i in range(0, 10)]
...
def test_load_by_cv2():
    ...
    # draw_plt(scrutiny(bayes, 10000), 'bayes2')
    draw_plt(scrutiny(bayes, 10000), 'bayes2')
...
def bayes(image):
    ...
    another = bayes2(image)[0]
    if another == 0:
        return another, max_prob
    else:
        return max_index, max_prob
...
def bayes2(image):
    ...
    max_prob = 0
    max_index = -1
    bayes_delta = [0 for i in range(0, 10)]
    for i in range(len(image)):
        # 二值化 只有0与1
        if image[i] > threshold:
            image[i] = 1
            ...
            image[i] = 0
    for i in range(10):
        prob_true = bayes_probability(image, bayes_contingent[i])
        prob_false = bayes_probability(image, bayes_false[i])
        bayes_delta[i] = prob_true - prob_false
    return bayes_delta.index(max(bayes_delta)), bayes_delta


def bayes_check(image, label):
    for i in range(len(image)):
        # 二值化 只有0与1
        if image[i] > threshold:
            image[i] = 1
            ...
            image[i] = 0
    prob_true = bayes_probability(image, bayes_contingent[label])
    prob_false = bayes_probability(image, bayes_false[label])
...
def bayes_init(size=60000):
    ...
    # 二分类false处理
    for i in range(10):
        bayes_false[i] /= (sum(bayes_count) - bayes_count[i])
