...
bayes_test = [np.ones(image_size) for i in range(2)]
test = 4
...
def test_load_by_cv2():
    ...
    bayes_init(20000)
    scrutiny(bayes, 1000)
    draw_plt(correct_count)
    print(euclidean(process_input(cv2.imread("image/4.jpg"))))
...
def bayes2(image):
    ...
    for i in range(len(image)):
        ...
        if image[i] > threshold:
            ...
            image[i] = 0
    for i in range(2):
        prob = bayes_probability(image, bayes_test[i])
        if max_prob < prob:
            max_index = i
            max_prob = prob
    if max_index == 0:
...
def bayes_init(size=60000):
    ...
    while i <= len(train_cache) - 1000:
        ...
        # TODO
        if label == test:
            bayes_test[0] += image_template
        else:
            bayes_test[1] += image_template
        # 文件图像步长为 28x28=image_size
        i += image_size
        account += 1
        if account > size:
    ...
    bayes_test[0] /= bayes_count[test]
    bayes_test[1] /= (account - bayes_count[test])
